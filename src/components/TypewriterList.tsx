...
export default function TypewriterList({ text, delay = 0, onAnimationComplete, onAnimationStart, scroll }: {
    text: string
    delay?: number
    ...
    const textsToRender = useMemo(() => {
        return texts.map((t, index) => (
            t.length > 0 ? <motion.span key={index}>
                {(t[0] == " ") && <motion.span> </motion.span>}
                <Typewriter
                    text={t}
                    index={index + t}
                    delay={delay}
                    onAnimationStart={texts.length - 1 == index ?
                        onAnimationStart : undefined}
                    onAnimationComplete={texts.length - 1 == index ?
